...
-- 1 showing delivery companies from Austria - name , and ZIP code .
...
-- 2 showing which  products have  quantity  more than 35.
...
-- 3 showing acounts with last time login from 01.01.2019 to 01.01.2022
...
-- 4 showing the purchases dates for customer with first name Marko

SELECT customers.first_name, purchases.purchase_date
FROM customers
  JOIN purchase_acount on purchase_acount.fk_acount_id = customers.customer_id
  JOIN purchases on purchases.purchase_id = purchase_acount.fk_purchase_id
WHERE first_name = "Marko";


-- 5 showing which companies are from Amstetten.
...
-- 6 showing the first name , last name, purcasing dates , product name, and product price for customer - Cristina.
...
-- 7 showing the list of customers first name, last name, product ordered, price, shipping date and delivery date.
...
-- 8 showing the list of customers - first and last name, purchase= date and quantity, product- name and price , from wich company us the product where quantyty of purchased products is more than one.
...
-- 9 selecting the name and price of the product company name, adress and city delivery and purchase date 
